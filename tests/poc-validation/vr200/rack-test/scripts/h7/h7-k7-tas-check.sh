...
set -uo pipefail
cd "$(dirname "$0")" || exit 1
LOG="${HOME}/mokka-hetero/logs/h7-k7-tas-check.log"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
...
kubectl -n "${NS}" get pods -l mokka-hetero.nvidia.com/kueue-job=tas-a -o json > /tmp/h7-tas-a-pods.json
kubectl get nodes -o json > /tmp/h7-nodes.json
jq -r --slurpfile nodes /tmp/h7-nodes.json '
  ...
  | "pods=\($p|length) running=\([$p[]|select(.phase=="Running")]|length) distinct_nodes=\([$p[].node]|unique|length) kwok_vr200_nodes=\([$p[].node|select(startswith("kwok-vr200-"))]|unique|length) cliques=\([$p[].node|$clq[.]]|unique) selector_eq_node=\(all($p[]; .sel==.node))"' /tmp/h7-tas-a-pods.json | tee /tmp/h7-t2.txt
...
grep -q "pods=18 running=18 distinct_nodes=18 kwok_vr200_nodes=18 cliques=\[\"${vr200_clique}\"\] selector_eq_node=true" /tmp/h7-t2.txt \
  && pass "T2 18 pods, 18 trays, one rack (${vr200_clique}), TAS hostname == bound node" || bad "T2 placement: $(cat /tmp/h7-t2.txt) want clique ${vr200_clique}"
# DRA agrees with TAS: each pod's claim is allocated 4 devices from its own node's pool
# H7: evidence dump only (pod -> claim -> pool/device rows for the report); the checks below read the API as before.
kubectl -n "${NS}" get resourceclaims -o json > /tmp/h7-tas-a-claims.json
mism="$(kubectl -n "${NS}" get resourceclaims -o json | jq -r --slurpfile pods /tmp/h7-tas-a-pods.json '
...
nclaims="$(kubectl -n "${NS}" get resourceclaims -o json | jq --slurpfile pods /tmp/h7-tas-a-pods.json '($pods[0].items|map(.metadata.uid)) as $u | [.items[] | select(.status.reservedFor[0].uid as $x | $u | index($x))] | length')"
