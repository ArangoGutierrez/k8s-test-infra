...
set -uo pipefail
LOG="${HOME}/mokka-hetero/logs/h8-c1-build-overlay.log"
...
fail=0
docker run --rm --entrypoint /usr/bin/nvidia-imex-ctl "${IMG}" -h > /tmp/h8-imexctl-help.txt 2>&1
echo "nvidia-imex-ctl -h rc=$?"; head -3 /tmp/h8-imexctl-help.txt
grep -qi 'usage\|imex' /tmp/h8-imexctl-help.txt || { echo "FAIL nvidia-imex-ctl does not run in the image"; fail=1; }
# H8: the grep above also matches "nvidia-imex-ctl: error while loading shared
# libraries", so resolve both real binaries with the base's own dynamic loader.
for b in /usr/bin/nvidia-imex.real /usr/bin/nvidia-imex-ctl; do
  docker run --rm --entrypoint /lib64/ld-linux-x86-64.so.2 "${IMG}" --list "${b}" > /tmp/h8-ldlist.txt 2>&1
  lrc=$?; echo "ld.so --list ${b} rc=${lrc}"; cat /tmp/h8-ldlist.txt
  if [[ ${lrc} -ne 0 ]] || grep -q 'not found' /tmp/h8-ldlist.txt; then echo "FAIL ${b} has unresolved libraries on this base"; fail=1; fi
done
grep -qi 'error while loading shared libraries' /tmp/h8-imexctl-help.txt && { echo "FAIL nvidia-imex-ctl misses libraries"; fail=1; }
...
docker run --rm --entrypoint /usr/bin/nvidia-imex.real "${IMG}" --help > /tmp/h8-imex-help.txt 2>&1
echo "nvidia-imex.real --help rc=$? (non-zero is acceptable, missing shared libraries are not)"
grep -i 'error while loading shared libraries' /tmp/h8-imex-help.txt && { echo "FAIL nvidia-imex.real misses libraries on this base"; fail=1; }
(( fail == 0 )) || exit 1
