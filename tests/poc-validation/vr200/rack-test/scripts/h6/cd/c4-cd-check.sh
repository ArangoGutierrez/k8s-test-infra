...
set -uo pipefail
LOG="${HOME}/mokka-hetero/logs/h6-c4-cd-check.log"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
NS=mokka-hetero-cd
DRV=nvidia
WANT_CLIQUE="00000000-0000-0000-0000-000000000001.32766"   # vr200.yaml:77-80 fabric block
read -r CH_MAJOR _ < "${HOME}/mokka-hetero/tmp/h6-imex-majors.txt"
...
kubectl -n "${NS}" rollout status deploy/vr200-cd-workload --timeout=300s || bad "workload not Ready"

echo "== C1 placement: 3 workload pods on worker7..9, each with 4 Rubin devices from its own node"
kubectl -n "${NS}" get pods -l app=vr200-cd-workload -o json > /tmp/h6-cd-pods.json
nodes="$(jq -r '[.items[] | select(.status.phase=="Running") | .spec.nodeName] | sort | join(",")' /tmp/h6-cd-pods.json)"
[[ "${nodes}" == "mokka-hetero-worker7,mokka-hetero-worker8,mokka-hetero-worker9" ]] && pass "C1 pods on ${nodes}" || bad "C1 pods on '${nodes}'"
gpu_ok="$(kubectl -n "${NS}" get resourceclaims -o json | jq --slurpfile p /tmp/h6-cd-pods.json '
...
kubectl -n "${DRV}" get computedomaincliques -o json | jq -r --arg u "${uid}" '.items[] | select(.metadata.name|startswith($u)) | "\(.metadata.name) \([.daemons[]? | "\(.nodeName)=\(.status)"] | sort | join(","))"' | tee /tmp/h6-cdclique.txt
[[ "$(wc -l < /tmp/h6-cdclique.txt)" -eq 1 ]] && grep -q "^${uid}.${WANT_CLIQUE} mokka-hetero-worker7=Ready,mokka-hetero-worker8=Ready,mokka-hetero-worker9=Ready$" /tmp/h6-cdclique.txt \
  && pass "C5 clique ${uid}.${WANT_CLIQUE} holds worker7..9 Ready" || bad "C5 clique objects: $(cat /tmp/h6-cdclique.txt)"
want_major="$(printf '%x' "${CH_MAJOR}")"
for p in $(jq -r '.items[].metadata.name' /tmp/h6-cd-pods.json); do
