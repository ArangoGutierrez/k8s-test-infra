...
set -uo pipefail
LOG="${HOME}/mokka-hetero/logs/h6-c2-enable-cd.log"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
...
echo "${CH_MAJOR} ${CAPS_MAJOR}" > "${HOME}/mokka-hetero/tmp/h6-imex-majors.txt"
...
kubectl get resourceslices -o json | jq -r '[.items[] | select(.spec.driver=="gpu.nvidia.com" and (.spec.nodeName|startswith("mokka-hetero-")))] | map(.spec.devices|length) | add' > /tmp/h6-gpu-devs-before.txt
...
[[ "${after}" == "$(cat /tmp/h6-gpu-devs-before.txt)" && "${after}" == "48" ]] && echo "PASS real-tier GPU devices still 48" || { echo "FAIL GPU devices before=$(cat /tmp/h6-gpu-devs-before.txt) after=${after}"; fail=1; }
