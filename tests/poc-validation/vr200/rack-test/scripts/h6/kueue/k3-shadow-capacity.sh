...
set -uo pipefail
LOG="${HOME}/mokka-hetero/logs/h6-k3-shadow-capacity.log"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
echo "start $(date -u +%FT%TZ)"
fail=0

kubectl get resourceslices -o json > /tmp/h6-slices.json
...
jq -r '[.items[] | select(.spec.driver=="gpu.nvidia.com" and .spec.nodeName != null)
       ...
       | group_by(.n) | .[] | "\(.[0].n) \(map(.c) | add)"' /tmp/h6-slices.json > /tmp/h6-slice-counts.txt

while read -r node gtype; do
  count="$(awk -v n="${node}" '$1==n{print $2}' /tmp/h6-slice-counts.txt)"
  ...
  rc=$?; [[ ${rc} -eq 0 ]] || { echo "FAIL patch ${node} rc=${rc}"; fail=1; }
                ...
                "\(.metadata.name) \(.metadata.labels["mokka-hetero.nvidia.com/gpu-type"])"' /tmp/h6-nodes.json)

sleep 5
kubectl get nodes -o json > /tmp/h6-nodes-after.json
echo "== verify: allocatable == slice devices on every KWOK node; no tas-* on real nodes"
jq -r --rawfile counts /tmp/h6-slice-counts.txt '
  ...
  | if .metadata.labels["type"]=="kwok" then
    ...
    end' /tmp/h6-nodes-after.json | tee /tmp/h6-shadow-verify.txt | sort | uniq -c -w 4
grep -q '^FAIL' /tmp/h6-shadow-verify.txt && { grep '^FAIL' /tmp/h6-shadow-verify.txt; fail=1; }
...
ipbad="$(jq -r '.items[] | select(.metadata.labels["type"]=="kwok")
  ...
  | select($ip | startswith("172.18.250.") | not) | "\(.metadata.name) \($ip)"' /tmp/h6-nodes-after.json)"
[[ -z "${ipbad}" ]] && echo "PASS all KWOK InternalIPs still 172.18.250.x" || { echo "FAIL InternalIP changed: ${ipbad}"; fail=1; }
echo "kwok nodes verified: $(grep -c '^PASS kwok-' /tmp/h6-shadow-verify.txt) (want 54)"
[[ "$(grep -c '^PASS kwok-' /tmp/h6-shadow-verify.txt)" -eq 54 ]] || fail=1
