...
set -uo pipefail
LOG="${HOME}/mokka-hetero/logs/h6-k2-kueue-install.log"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
...
echo "== live webhooks: every namespaceSelector must be the test namespace (cluster-scoped Kueue objects have none)"
bad="$(kubectl get mutatingwebhookconfigurations,validatingwebhookconfigurations -o json | jq -r '
  .items[] | select(.metadata.name | test("kueue")) | .webhooks[] |
  select(.namespaceSelector != null and .namespaceSelector != {"matchLabels":{"kubernetes.io/metadata.name":"mokka-hetero-kueue"}}) |
  "\(.name) \(.failurePolicy) \(.namespaceSelector|tostring)"')"
...
kubectl -n kueue-system get cm kueue-manager-config -o jsonpath='{.data.controller_manager_config\.yaml}' > /tmp/h6-kueue-cfg.yaml
grep -A3 'deviceClassMappings' /tmp/h6-kueue-cfg.yaml
grep -q 'mokka-hetero.nvidia.com/dra-gpu' /tmp/h6-kueue-cfg.yaml || { echo "FAIL mapping missing"; fail=1; }
